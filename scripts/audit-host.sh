...
[ "$runtime_policy" = 'true|true|false|mobile-wireguard-egress||["ALL"]|["CAP_DAC_READ_SEARCH","CAP_NET_ADMIN","CAP_SETGID","CAP_SETUID"]|["no-new-privileges:true"]|32' ] || \
...
filter_rules="$(docker exec "$container_id" nft --numeric list table inet vpn_filter)"
[ "$(printf '%s\n' "$filter_rules" | grep -Fc 'policy drop;')" -eq 2 ] || \
    die "the namespace firewall must have exactly two default-drop policies"
[ "$(printf '%s\n' "$filter_rules" | grep -Fc 'iifname "eth0" udp dport 443 accept')" -eq 1 ] || \
...
printf '%s\n' "$filter_rules" | grep -F 'iifname "wg0" oifname "eth0" ip saddr 10.77.0.0/24 accept' >/dev/null
printf '%s\n' "$filter_rules" | grep -F 'iifname "eth0" oifname "wg0" ip daddr 10.77.0.0/24 ct state' >/dev/null
nat_rules="$(docker exec "$container_id" nft --numeric list table ip vpn_nat)"
printf '%s\n' "$nat_rules" | grep -F 'oifname "eth0" ip saddr 10.77.0.0/24 masquerade' >/dev/null
